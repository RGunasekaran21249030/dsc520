...
setwd("C:/Users/ragun/Documents/GitHub/dsc520")
...
## Check Top 10 rows
head(heights_df, n=10)

## Check Bottom-Last 10 rows
tail(heights_df, n=10)
...
## `age` vs. `earn`
ggplot(heights_df, aes(x=age , y=earn)) + geom_smooth()
## `ed` vs. `earn`
ggplot(heights_df, aes(x=ed , y=earn)) + geom_smooth()
...
ggplot(heights_df, aes(x=height , y=earn, col=sex))+ geom_smooth() +ggtitle("Height vs. Earnings") +xlab("Height (Inches)") +ylab("Earnings (Dollars)")
...
ggplot(heights_df, aes(y=earn)) + geom_density()
